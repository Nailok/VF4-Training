FrameData = {}
FrameData.values = {}
FrameData.values.temp = {}

FrameData.values.p1_startup = 0
FrameData.values.tmp_p1_startup = 0
...
function FrameData.p1_startup()
    ...
    -- if PlayersInfo.hit_state.hit_active(1) and move_startup ~= 1 then
    --     FrameData.values.tmp_p1_startup = move_startup
    -- end

    -- if PlayersInfo.got_hit(2) then
    if PlayersInfo.test_p2_hit() then
        FrameData.values.p1_startup = move_startup
    end

    -- if PlayersInfo.player_juggled(2) or PlayersInfo.player_thrown(2) then
...
function FrameData.p1_combo_damage()
    ...
    if combo_damage ~= 0 then   --- 6p on crouching opponent shows zero for some reason
...
function FrameData.hit_type()
    ...
    --     FrameData.values.hit_type = 'In air (mid)'
    -- elseif hit_type == 5 then
    --     FrameData.values.hit_type = 'In air (low)'
    ...
    if PlayersInfo.player_thrown(1) or PlayersInfo.player_thrown(2) then
        FrameData.values.hit_type = 'Throw'
        MEMORY.write8(GAME_ADDRESSES.hit_type, 100)
    end
